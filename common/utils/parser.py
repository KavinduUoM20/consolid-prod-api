...
def parse_with_mistral_from_bytes(file_bytes: bytes, filename: str, mime_type: str = None) -> Optional[str]:
    ...
    Returns the markdown content directly, not a file path
    """
    try:
        ...
        # Call the original function but capture the markdown content instead of saving to file
        return parse_with_mistral_content_only(mock_file)
...
def parse_with_mistral_content_only(file: UploadFile) -> str:
    """
    Extracts content from an uploaded file (PDF or Image) using Mistral OCR.
    - Reads file bytes
    - Encodes the file in base64
    - Constructs Mistral-compatible payload depending on file type
    - Processes it and returns Markdown content directly (does not save to file)
    """
    try:
        # Read file bytes
        file_bytes = file.file.read()
        ...
        print(f"Markdown content generated successfully")
        return markdown
